...
parser.add_argument('--output_root', type=str, default='/content/output',
...
parser.add_argument('--data_folder', type=str, default='/content/MoCo/data')
...
if config.dataset == 'icbhi':
  if config.class_split == 'lungsound':
    if config.n_cls == 4:
      config.cls_list = ['normal', 'crackle', 'wheeze', 'both']
    elif config.n_cls == 2:
      config.cls_list = ['normal', 'abnormal']
    elif config.class_split == 'diagnosis':
      if config.n_cls == 3:
        config.cls_list = ['healthy', 'chronic_diseases', 'non-chronic_diseases']
    elif config.n_cls == 2:
      config.cls_list = ['healthy', 'unhealthy']
    else:
...
output_path = os.path.join(config.output_root, config.dataset, config.exp_version)
...
dloader, dlen = data_loader(config,batch_size=config.batch_size,
                            num_workers=config.num_workers,
                            type='encoder_train')

# Build models
print('\n[2 / 3]. Build models.. ')
encoder = nn.DataParallel(model.BiLSTMModel(hidden_size=config.feature_dim)).to(dev)
momentum_encoder = nn.DataParallel(model.BiLSTMModel(hidden_size=config.feature_dim)).to(dev)
...
with torch.no_grad():
    for i, (_, img, _) in enumerate(dloader):
        # print(f'shape of img just before feeding the mom_encoder {img.shape}')
